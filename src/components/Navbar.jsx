...
import { Activity, LogOut, User, LayoutDashboard, FileText, PlusCircle, Menu, X, Calendar, Edit, Bell, Pill, Lock, Stethoscope, MapPin, Syringe, Sun, Moon, Activity as ScoreIcon } from 'lucide-react'
...
export default function Navbar({ unreadCount = 0 }) {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-slate-800 bg-slate-950/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-0.5 overflow-x-auto max-w-3xl">
          {links.map(({ to, label, icon: Icon, badge }) => (
            <Link key={to} to={to}
              className={`relative flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${isActive(to) ? 'bg-teal-500/15 text-teal-300 border border-teal-500/25' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
        ...
        <div className="flex items-center gap-2">
          {/* Theme toggle */}
          <button onClick={toggleTheme} className="p-2 text-slate-500 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors" title="Toggle theme">
      ...
      {menuOpen && (
        <div className="lg:hidden border-t border-slate-800 bg-slate-950/95 px-4 py-3 flex flex-col gap-1 animate-fade-in max-h-96 overflow-y-auto">
          {links.map(({ to, label, icon: Icon, badge }) => (
