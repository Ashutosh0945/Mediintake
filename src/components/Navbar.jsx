    ...
    <nav style={{ background: 'rgba(2,6,23,0.96)', borderBottom: '1px solid rgba(8,145,178,0.22)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 4px 24px rgba(0,0,0,0.45)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* User pill */}
          <div className="hidden sm:flex items-center gap-2" style={{ background: 'rgba(8,145,178,0.1)', border: '1px solid rgba(8,145,178,0.2)', borderRadius: '10px', padding: '6px 12px' }}>
          ...
          <button onClick={handleSignOut}
            style={{ background: 'rgba(8,145,178,0.1)', border: '1px solid rgba(8,145,178,0.22)', borderRadius: '8px', padding: '7px', color: 'rgba(103,232,249,0.4)', cursor: 'pointer', transition: 'all 0.2s', display: 'flex' }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background='rgba(8,145,178,0.1)'; e.currentTarget.style.color='rgba(103,232,249,0.4)'; e.currentTarget.style.borderColor='rgba(8,145,178,0.22)' }}>
            <LogOut style={{ width: '14px', height: '14px' }} />
          </button>

          {/* Mobile hamburger */}
          <button className="lg:hidden" onClick={() => setMenuOpen(!menuOpen)}
            style={{ background: 'rgba(8,145,178,0.1)', border: '1px solid rgba(8,145,178,0.22)', borderRadius: '8px', padding: '7px', color: '#67E8F9', cursor: 'pointer', display: 'flex' }}>
      ...
      {menuOpen && (
        <div style={{ background: 'rgba(2,6,23,0.99)', borderTop: '1px solid rgba(8,145,178,0.18)' }} className="lg:hidden px-4 py-3 flex flex-col gap-1 animate-fade-in max-h-[80vh] overflow-y-auto">
          {links.map(({ to, label, icon: Icon, badge }) => (
            <Link key={to} to={to} onClick={() => setMenuOpen(false)} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, transition: 'all 0.15s',
              background: isActive(to) ? 'rgba(8,145,178,0.16)' : 'transparent',
              color: isActive(to) ? '#67E8F9' : 'rgba(103,232,249,0.38)',
