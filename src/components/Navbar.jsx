...
import { Activity, LogOut, User, LayoutDashboard, FileText, PlusCircle, Menu, X, Calendar, Edit, Bell, Pill, Lock, Stethoscope, MapPin } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-0.5">
          {links.map(({ to, label, icon: Icon, badge }) => (
            <Link key={to} to={to}
              className={`relative flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-xs font-medium transition-colors ${isActive(to) ? 'bg-teal-500/15 text-teal-300 border border-teal-500/25' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`}>
